...
export default class $2nd_AnimatedQuestionnaire extends Component {
  ...
  render() {
    ...
    const noMainQuestionInterpolate = this.state.animation.interpolate({
      inputRange: [0, 1],
      outputRange: [0, height]
    });

    const yesMainQuestionInterpolate = this.state.animation.interpolate({
      inputRange: [0, 1],
      outputRange: [0, -height]
    });
    ...
    const noMainQuestionStyle = {
      transform: [
        {
          translateY: noMainQuestionInterpolate
        }
      ]
    };
    const yesMainQuestionStyle = {
      transform: [
        {
          translateY: yesMainQuestionInterpolate
        }
      ]
    };
        ...
        <View style={[styles.overlay, StyleSheet.absoluteFill]}>
          <Animated.Text
            style={[
              styles.questionText,
              {
                ...(this.state.No ? noMainQuestionStyle : yesMainQuestionStyle)
              }
            ]}
          >
            {question}
